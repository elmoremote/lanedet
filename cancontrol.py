...
class CanControl():
    car_angle = 0
    def send_one(self, id, data):
        with can.Bus(interface="socketcan", channel="can1", bitrate=500000) as bus:
    ...
    def send_angle_2_car(self, angle):
        ...
        data = pos.to_bytes(1, 'big')+abs(int(angle)).to_bytes(2, 'big')
        self.send_one(id=0x01000003, data=[1,0])
        self.send_one(id=0x01010100, data=data)    
        self.send_one(id=0x01010007, data=[1])
        self.send_one(id=0x01030000, data=[0])
        self.send_one(id=0x01020000, data=[5])
